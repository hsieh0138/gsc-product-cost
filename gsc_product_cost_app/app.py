...
if not st.session_state.authenticated:
    ...
    # 匯率設定
    usd_rate = st.number_input("💱 請輸入今日匯率（1 USD = NTD）", value=31.5, step=0.1)
    ...
    default_data = pd.DataFrame({
        ...
        "品管成本 QC Cost": [5, 3],
        "毛利率% Profit Margin (%)": [20, 25],  # 使用百分比輸入
    })
    ...
    for _, row in edited_df.iterrows():
        if row["產品名稱 Product"]:
            ...
            results.append({
                ...
                "毛利率 (%)": row["毛利率% Profit Margin (%)"],
                "建議售價 (NTD)": suggested_price,
                "利潤額 Profit (NTD)": profit_amount,
                "建議售價 (USD)": usd_price,
            })
    ...
    if results:
        ...
        filename = f"川浩產品_成本試算結果_{now_str}.csv"
